...
public:
	virtual bool RegisterConCommandBase(ConCommandBase *pVar)
	{
		...
		i_Cvar->RegisterConCommand(pVar);
		return true;
	}
};
...
{
	if (!i_Cvar || s_bRegistered)
		return;

	assert(s_nDLLIdentifier < 0);
	s_bRegistered = true;
	s_nCVarFlag = nCVarFlag;
	s_nDLLIdentifier = i_Cvar->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
	...
	assert(s_nDLLIdentifier >= 0);
	i_Cvar->UnregisterConCommands(s_nDLLIdentifier);
	s_nDLLIdentifier = -1;
	s_bRegistered = false;
}
...
{
	if (i_Cvar) {
		i_Cvar->UnregisterConCommand(this);
	}
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
	...
	if (i_Cvar)
		i_Cvar->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
...
{
	m_pOriginalCVar = i_Cvar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::SpoofedConvar(ConVar* pCVar)
{
	...
	if (!IsSpoofed() && m_pOriginalCVar)
	{
		...
		//Register it
		i_Cvar->RegisterConCommand(m_pDummyCVar);
...
void SpoofedConvar::Restore()
{
	if (IsSpoofed())
	{
		...
		i_Cvar->UnregisterConCommand(m_pDummyCVar);
